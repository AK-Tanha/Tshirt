  ...
  return (
    <motion.header
      className={`fixed top-0 w-full z-50 transition-all duration-500 ${
        isScrolled
          ? "bg-white/80 backdrop-blur-md py-1 border-b border-border"
          : "bg-transparent py-2"
      }`}
    >
      <motion.div
                  ...
                  {categories?.map((cat) => (
                    <div key={cat.slug} className="flex-1">
                      ...
                      <div className="flex flex-col gap-3">
                        {products
                          .filter((item) => item.category?.slug === cat.slug)
                          .slice(0, 3)
                          .map((item) => (
                            <Link
                              key={item.id}
                            ...
                            >
                              ...
                              <div className="min-w-0">
                                <span className="font-body text-xs text-black group-hover:text-muted transition-colors block truncate">
                                  {item.name}
                                </span>
