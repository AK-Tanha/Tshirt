...
export default function Sidebar({ collapsed, onToggle }: { collapsed: boolean; onToggle: () => void }) {
  ...
  return (
    <aside
      className={cn(
        'fixed top-0 left-0 h-full z-30 flex flex-col bg-white dark:bg-neutral-950 border-r border-border transition-all duration-300',
        collapsed ? 'w-16' : 'w-64',
      )}
    >
      <div className={cn('flex items-center h-16 px-4 border-b border-border', collapsed && 'justify-center')}>
        <Link href="/admin" className="flex items-center gap-2.5 min-w-0">
          <div className="w-8 h-8 rounded-lg bg-black dark:bg-white flex items-center justify-center shrink-0">
            <Store className="w-4 h-4 text-white dark:text-black" />
          </div>
          {!collapsed && (
                        ...
                        {item.children!.map((child) => (
                          <Link
                            ...
                            className={cn(
                              ...
                              child === matchedChild
                                ? 'text-neutral-900 dark:text-white font-medium bg-neutral-100 dark:bg-neutral-900'
      ...
      <div className="p-2 border-t border-border">
        <button
          onClick={onToggle}
          className="flex items-center gap-3 w-full px-3 py-2 rounded-lg text-sm text-neutral-500 dark:text-neutral-400 hover:bg-neutral-100 dark:hover:bg-neutral-900 transition-colors"
        >
        ...
        <Link
          href="/"
          className={cn(
            'flex items-center gap-3 w-full px-3 py-2 rounded-lg text-sm text-neutral-500 dark:text-neutral-400 hover:bg-neutral-100 dark:hover:bg-neutral-900 transition-colors',
