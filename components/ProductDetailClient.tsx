    ...
    <div className="grid lg:grid-cols-12 gap-12">
      ...
      <MotionSection className="lg:col-span-5">
        <div className="sticky top-32">
            ...
            <div className="flex gap-2">
              {['S', 'M', 'L', 'XL', 'XXL'].map(size => (
                <button
                  ...
                  className={`w-10 h-10 border font-mono text-[10px] transition-all ${
                    selectedSize === size
                      ? 'border-forest bg-forest text-bone'
                      : 'border-slate/20 hover:border-forest hover:bg-forest hover:text-bone'
                  }`}
                >
                  {size}
                </button>
          ...
          <GarmentTag size={selectedSize} color="NAVY / FOREST" sku={product.id.toUpperCase()} className="mb-8 w-full" />
          ...
          <div className="flex flex-col gap-4">
            <button
              ...
              className="w-full bg-forest text-bone py-5 font-body uppercase text-xs tracking-widest hover:bg-navy transition-all duration-500 shadow-xl shadow-forest/10"
            >
              Add to Bag
            </button>
            <a
              href={`https://m.me/yourbrand?text=I%20want%20to%20order:%20${product.name}%20%E2%80%93%20Size%20${selectedSize}%20%E2%80%93%20%E2%9A%B1${product.price}`}
