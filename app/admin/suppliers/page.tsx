...
export default function AdminSuppliers() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="hidden md:block bg-white dark:bg-neutral-900 rounded-xl border border-border overflow-hidden">
          ...
          <table className="w-full">
            ...
            <tbody>
              {filtered.map((s) => {
                ...
                return (
                  <tr key={s.id} className="border-t border-border text-sm hover:bg-neutral-50 dark:hover:bg-neutral-900/50 transition-colors cursor-pointer" onClick={() => setSelectedSupplier(s)}>
                    <td className="p-4">
                      <div>
                    ...
                    <td className="p-4">
                      <div className="flex gap-1.5">
                        {s.supplyCategories.map((cat) => (
                          <span key={cat} className="text-[10px] font-medium bg-neutral-100 dark:bg-neutral-800 px-2 py-0.5 rounded-md capitalize">
                    ...
                    <td className="p-4">
                      <span className="text-neutral-400 hover:text-neutral-900 dark:hover:text-white transition-colors text-xs font-medium cursor-pointer">
                        View
                      </span>
                    </td>
                  </tr>
      ...
      <div className="md:hidden space-y-3">
        {filtered.map((s) => {
          ...
          return (
            <div
              key={s.id}
              className="bg-white dark:bg-neutral-900 rounded-xl border border-border p-4 space-y-3 cursor-pointer"
        ...
        {filtered.length === 0 && (
          <div className="bg-white dark:bg-neutral-900 rounded-xl border border-border p-12 text-center text-neutral-500 text-sm">No suppliers found</div>
        )}
      </div>
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="absolute right-0 top-0 h-full w-full max-w-lg bg-white dark:bg-neutral-900 border-l border-border shadow-xl overflow-y-auto"
      >
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="flex items-center justify-between px-4 py-3 bg-neutral-100 dark:bg-neutral-800 rounded-lg">
          ...
          <div>
            ...
            <div className="flex gap-2">
              {supplier.supplyCategories.map((cat) => (
                <span key={cat} className="text-xs font-medium bg-neutral-900 dark:bg-white text-white dark:text-neutral-900 px-3 py-1.5 rounded-lg capitalize">
                  {cat === 'tshirt' ? 'T-Shirt' : cat}
                </span>
              ))}
            </div>
            ...
            <div className="space-y-2">
              {supplierProducts.map((p) => (
                <Link
                  ...
                  onClick={onClose}
                  className="flex items-center justify-between px-4 py-3 bg-neutral-100 dark:bg-neutral-800 rounded-lg hover:bg-neutral-200 dark:hover:bg-neutral-700 transition-colors"
                >
          ...
          <div className="flex gap-3 pt-2">
            <button
              onClick={onClose}
              className="flex-1 px-4 py-2.5 border border-border text-sm font-medium rounded-lg hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-colors"
            >
              Close
            </button>
            <button className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 bg-neutral-900 text-white dark:bg-white dark:text-neutral-900 text-sm font-medium rounded-lg hover:bg-neutral-800 dark:hover:bg-neutral-200 transition-colors">
...
function SupplierStatCard({ title, value, subtitle, icon: Icon, color }: {
  ...
  return (
    <motion.div
      ...
      className="bg-white dark:bg-neutral-900 rounded-xl border border-border p-4 md:p-5"
    >
