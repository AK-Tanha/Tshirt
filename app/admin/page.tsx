...
export default function AdminDashboard() {
  ...
  const kpis = [
    { title: 'Revenue Today', value: '৳12,450', change: 12.5, icon: DollarSign, color: 'bg-emerald-100 text-emerald-600 dark:bg-emerald-950/40 dark:text-emerald-400' },
    { title: 'Revenue This Month', value: `৳${totalRevenue.toLocaleString()}`, change: 8.3, icon: TrendingUp, color: 'bg-emerald-100 text-emerald-600 dark:bg-emerald-950/40 dark:text-emerald-400' },
    { title: 'Orders Today', value: '24', change: 15.2, icon: ShoppingCart, color: 'bg-blue-100 text-blue-600 dark:bg-blue-950/40 dark:text-blue-400' },
    { title: 'Pending Orders', value: adminStats.pendingOrders.toString(), change: -2.1, icon: Clock, color: 'bg-amber-100 text-amber-600 dark:bg-amber-950/40 dark:text-amber-400' },
    { title: 'Completed Orders', value: '98', change: 6.7, icon: Package, color: 'bg-emerald-100 text-emerald-600 dark:bg-emerald-950/40 dark:text-emerald-400' },
    { title: 'New Customers', value: '32', change: 18.5, icon: Users, color: 'bg-purple-100 text-purple-600 dark:bg-purple-950/40 dark:text-purple-400' },
    { title: 'Total Products', value: adminStats.totalProducts.toString(), change: 0, icon: Package, color: 'bg-neutral-100 text-neutral-600 dark:bg-neutral-800 dark:text-neutral-400' },
    { title: 'Low Stock Items', value: '4', change: -5.0, icon: AlertTriangle, color: 'bg-red-100 text-red-600 dark:bg-red-950/40 dark:text-red-400' },
  ];
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-neutral-500 dark:text-neutral-400 mt-1">Your store at a glance</p>
        </div>
        <span className="text-xs text-neutral-500 bg-neutral-100 dark:bg-neutral-900 px-3 py-1.5 rounded-lg font-mono">FY 2026</span>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="flex items-end gap-2 h-48 min-w-[480px]">
              {revenueByMonth.map((item, idx) => (
                <div key={item.month} className="flex-1 flex flex-col items-center gap-1.5 group">
                  <span className="text-[10px] text-neutral-500 opacity-0 group-hover:opacity-100 transition-all font-mono bg-neutral-100 dark:bg-neutral-800 px-1.5 py-0.5 rounded -mt-6">
                    ৳{item.revenue.toLocaleString()}
                  </span>
                  <div className="w-full relative flex-1 flex items-end">
                    <div
                      className="w-full rounded-sm cursor-pointer transition-all duration-300 dark:bg-neutral-700"
                      style={{
                  ...
                  <span className={cn('text-[10px] font-medium', idx === revenueByMonth.length - 1 ? 'text-neutral-900 dark:text-white' : 'text-neutral-500')}>
                    {item.month}
                  </span>
                </div>
              ))}
        ...
        <Card className="p-6">
            ...
            {categoryBreakdown.map((cat) => (
              <div key={cat.category}>
                ...
                <div className="h-2 bg-neutral-100 dark:bg-neutral-800 rounded-full overflow-hidden">
                  <div className="h-full bg-neutral-900 dark:bg-white rounded-full transition-all" style={{ width: `${cat.percentage}%` }} />
                </div>
          ...
          <Link href="/admin/products" className="inline-flex items-center gap-1 text-xs text-neutral-500 hover:text-neutral-900 dark:hover:text-white transition-colors mt-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-2" padding={false}>
          <div className="flex items-center justify-between p-6 border-b border-border">
            ...
            <Link href="/admin/orders" className="text-xs text-neutral-500 hover:text-neutral-900 dark:hover:text-white transition-colors flex items-center gap-1">
              ...
              <div key={product.name} className="flex items-center justify-between">
                <div className="flex items-center gap-3 min-w-0">
                  <span className={cn(
                    ...
                    i === 0 ? 'bg-neutral-900 text-white dark:bg-white dark:text-neutral-900' : 'bg-neutral-100 dark:bg-neutral-800 text-neutral-500',
                  )}>
                    {i + 1}
                  </span>
