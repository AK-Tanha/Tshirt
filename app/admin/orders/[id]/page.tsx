...
export default function OrderDetailPage() {
 ...
 i <= currentIdx ? 'bg-neutral-900 text-white ' : 'bg-neutral-100 text-neutral-500',
 )}>
 {i < currentIdx ? <Check className="w-4 h-4" /> : i + 1}
 </div>
 {i < statusFlow.length - 1 && (
 <div className={cn('flex-1 h-[3px] rounded-full', i < currentIdx ? 'bg-neutral-900 ' : 'bg-border')} />
 )}
 </div>
 ))}
 </div>
 ...
 {nextStatus && (
 <button onClick={() => updateStatus(nextStatus)} className="flex items-center gap-2 px-4 py-2.5 bg-neutral-900 text-white text-sm font-medium rounded-lg hover:bg-neutral-800 transition-colors">
 ...
 <button onClick={() => updateStatus('returned')} className="flex items-center gap-2 px-4 py-2.5 border border-border text-sm font-medium rounded-lg hover:bg-neutral-100 transition-colors">
 ...
 rows={3}
 className="w-full px-4 py-2.5 bg-neutral-100 border-none rounded-lg text-sm outline-none focus:ring-2 focus:ring-neutral-300 transition-all resize-none placeholder:text-neutral-400"
 />
 </Card>
 </div>
